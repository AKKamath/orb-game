...
var difficulty : int = DIFF.NORMAL
var style : int = STYLE.CLASSIC
...
func _on_resized() -> void:
	...
	# STYLE: 30% vertical space
	$StyleUI.position.x = (vp.x - $StyleUI.size.x) / 2
	$StyleUI.position.y = y_off + (vp.y * 0.3) / 2
	y_off += 0.3 * vp.y
	
	# DIFFICULTY: 10% vertical space, keep it close to difficulty
...
func _on_start_button_pressed() -> void:
	...
	var level = root.get_node("TitleUI")
